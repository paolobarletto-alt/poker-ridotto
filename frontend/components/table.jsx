// Table — live poker table with animated dealing/betting
const { useState: useStateT, useEffect: useEffectT, useRef: useRefT } = React;

const SUITS = { '♠': '#0a0a0a', '♥': '#c0392b', '♦': '#c0392b', '♣': '#0a0a0a' };

// ————— Card back patterns (for Tweaks) —————
const CARD_BACKS = {
  'classico': {
    name: 'Classico Rosso',
    render: () => (
  ...
  'ridotto': {
    ...
    render: () => (
      <div style={{
        ...
        <div style={{
          position: 'absolute', inset: 4, border: '0.5px solid rgba(212,175,55,0.4)',
        }} />
  ...
  'minimale': {
    ...
    render: () => (
      <div style={{
        ...
        <div style={{
          width: 3, height: 3, background: '#D4AF37', borderRadius: '50%',
        }} />
      </div>
    ),
  },
...
function Card({ card, size = 'md', cardBack = 'ridotto' }) {
  const sizes = {
  ...
  if (!card || card === 'back') {
    return (
      <div style={{
        width: s.w, height: s.h, borderRadius: 5, overflow: 'hidden',
  ...
  return (
    <div style={{
      ...
      <div style={{
        position: 'absolute', bottom: 5, right: 6, fontSize: s.suit, color,
        lineHeight: 1,
      }}>{suit}</div>
    </div>
  );
  ...
  if (!seat) {
    return (
      <div style={{
        ...
        width: 130, transform: 'translate(-50%, -50%)',
        textAlign: 'center',
      }}>
        <div style={{
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          color: acting ? '#D4AF37' : 'rgba(245,241,232,0.75)',
        }}>€{seat.stack.toLocaleString('it-IT')}</div>
      </div>
      ...
      {seat.lastAction && !folded && (
        <div style={{
          marginTop: 4, fontSize: 9.5, letterSpacing: '0.2em', fontWeight: 600,
      ...
      {folded && (
        <div style={{
          marginTop: 4, fontSize: 9.5, letterSpacing: '0.2em', fontWeight: 600,
          color: 'rgba(245,241,232,0.5)', fontFamily: 'Inter, sans-serif',
        }}>FOLD</div>
      )}
      ...
      {seat.bet > 0 && !folded && (
        <div style={{
          position: 'absolute', ...seat.betOffset,
...
// ————— Table component —————
function PokerTable({ cardBack, onLeave }) {
  const [pot, setPot] = useStateT(0);
  const [community, setCommunity] = useStateT([]);
  const [phase, setPhase] = useStateT('preflop'); // preflop, flop, turn, river, showdown
  const [heroAction, setHeroAction] = useStateT(null);
  const [raiseAmt, setRaiseAmt] = useStateT(40);
  ...
  // Seat positions on an oval table (percentages of table container)
  const seats = [
  ...
  // Auto-deal simulation — advances phases over time
  useEffectT(() => {
    // initial: pot from blinds + calls + raise
    const initialPot = 5 + 10 + 40 + 40;
    setPot(initialPot);

    const phases = [
      { at: 2500, do: () => { setCommunity(['10♥', 'J♠', 'Q♦']); setPhase('flop');
        setLog(l => [{ t: '21:34:18', txt: 'FLOP: 10♥  J♠  Q♦' }, ...l]); } },
      { at: 5500, do: () => { setCommunity(c => [...c, '9♣']); setPhase('turn');
        setLog(l => [{ t: '21:34:24', txt: 'TURN: 9♣' }, ...l]); } },
      { at: 8500, do: () => { setCommunity(c => [...c, '2♠']); setPhase('river');
        setLog(l => [{ t: '21:34:30', txt: 'RIVER: 2♠' }, ...l]); } },
    ];
  ...
  return (
    <div style={{
      ...
      background: 'radial-gradient(ellipse at center, #0a1810 0%, #050a07 100%)',
    }}>
      {/* Header bar */}
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
          <div style={{ fontSize: 11, color: 'rgba(245,241,232,0.7)' }}>
            <span style={{ color: 'rgba(245,241,232,0.45)' }}>Mano</span> <span style={{ fontFamily: 'JetBrains Mono, monospace' }}>#4,182,093</span>
          </div>
          <div style={{ fontSize: 11, color: 'rgba(245,241,232,0.7)' }}>
            <span style={{ color: 'rgba(245,241,232,0.45)' }}>Seduti</span> <span style={{ fontFamily: 'JetBrains Mono, monospace' }}>6/8</span>
          </div>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{
                ...
                color: 'rgba(212,175,55,0.15)', fontStyle: 'italic', letterSpacing: '0.02em',
              }}>Ridotto</div>
              ...
              <div style={{
                ...
                <div style={{
                  fontSize: 9, letterSpacing: '0.25em', color: 'rgba(245,241,232,0.5)',
                  fontFamily: 'Inter, sans-serif', marginBottom: 4,
                }}>PIATTO</div>
                <div style={{
                  fontFamily: 'Playfair Display, serif', fontSize: 24, color: '#D4AF37',
                  fontWeight: 500,
                }}>€{pot.toLocaleString('it-IT')}</div>
              </div>
              ...
              <div style={{
                ...
                {[0, 1, 2, 3, 4].map(i => (
                  <div key={i} style={{
                    transition: 'all 0.4s',
                    transform: community[i] ? 'translateY(0)' : 'translateY(-8px)',
                    opacity: community[i] ? 1 : 0,
                  }}>
                    {community[i] ? <Card card={community[i]} size="md" cardBack={cardBack} /> :
            ...
            {/* Seats */}
            {seats.map(s => (
              <Seat key={s.id} seat={s} pos={s.pos} isHero={s.isHero}
                    dealerBtn={s.id === 7} cardBack={cardBack} />
            ))}
          </div>
        ...
        <div style={{
          ...
          <div style={{
            padding: '14px 18px', borderBottom: '1px solid rgba(212,175,55,0.1)',
          }}>
            <div style={{ fontSize: 10, letterSpacing: '0.22em', color: '#D4AF37', fontWeight: 600 }}>
              STORICO MANO
            </div>
          </div>
            ...
            {log.map((l, i) => (
              <div key={i} style={{
                ...
                <span style={{
                  ...
                  color: 'rgba(245,241,232,0.4)', marginRight: 8,
                }}>{l.t}</span>
                {l.txt}
              </div>
            ))}
...
Object.assign(window, { PokerTable, Card, CARD_BACKS });

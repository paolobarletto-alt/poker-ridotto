...
export function Card({ card, size = 'md', cardBack = 'ridotto' }) {
  if (!card || card === 'back') return <div style={{ width: 52, height: 74, borderRadius: 6, background: '#222' }}>{CARD_BACKS[cardBack].render()}</div>;
  const rank = card.slice(0, -1);
  const suit = card.slice(-1);
  const color = SUITS[suit] || '#000';
  return (
    <div style={{ width: 52, height: 74, borderRadius: 6, background: '#fff', padding: 6 }}>
      <div style={{ color }}>{rank}</div>
      <div style={{ color }}>{suit}</div>
    </div>
  );
}

function ChipStack({ amount }) { if (!amount) return null; return <div style={{ width: 20 }}>{amount}</div>; }
...
function Seat({ seat, pos, isHero, dealerBtn, cardBack }) {
  if (!seat) return (
    <div style={{ position: 'absolute', ...pos, width: 130, transform: 'translate(-50%,-50%)', textAlign: 'center' }}>
      <div style={{ width: 62, height: 62, borderRadius: '50%', border: '1px dashed rgba(212,175,55,0.3)' }}>VUOTO</div>
    </div>
  );

  const folded = seat?.status === 'folded' || seat.folded;
  const acting = seat?.acting;

  return (
    <div style={{ position: 'absolute', ...pos, transform: 'translate(-50%,-50%)', width: 150, textAlign: 'center', opacity: folded ? 0.4 : 1 }}>
      {seat.cards && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: 4, marginBottom: 6 }}>
          {seat.cards.map((c, i) => <Card key={i} card={isHero ? c : 'back'} size={isHero ? 'md' : 'sm'} cardBack={cardBack} />)}
        </div>
      )}
      <div style={{ padding: '8px 12px', background: acting ? 'rgba(212,175,55,0.08)' : 'rgba(0,0,0,0.6)', borderRadius: 8 }}>
        <div style={{ color: '#fff', fontSize: 12, overflow: 'hidden', textOverflow: 'ellipsis' }}>{seat.name}</div>
        <div style={{ color: acting ? '#D4AF37' : '#ddd', fontFamily: 'JetBrains Mono, monospace' }}>€{seat.stack}</div>
      </div>
      {seat.bet > 0 && !folded && <div style={{ marginTop: 6 }}><ChipStack amount={seat.bet} /></div>}
    </div>
  );
}
...
export default function PokerTable({ tableId, cardBack = 'ridotto', onLeave }) {
  ...
  const handleAction = (action, payload = {}) => {
    ...
    const txt = action.toUpperCase();
    setLog(l => [{ t: time, txt }, ...l]);
    if (sendAction) sendAction(action, payload.amount ?? 0);
  };

  const handleSendChat = () => { if (!chatInput) return; if (sendMessage) sendMessage(chatInput); setChatInput(''); };

  const seats = (tableState && tableState.seats) ? tableState.seats : Array.from({ length: 8 }).map((_, i) => null);
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: 14, borderBottom: '1px solid rgba(0,0,0,0.05)', display: 'flex', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <div style={{ fontSize: 12 }}>Mano <span style={{ fontFamily: 'JetBrains Mono, monospace' }}>{tableState?.handId ?? '-'}</span></div>
          <div style={{ fontSize: 12 }}>{(tableState?.seats || seats).filter(Boolean).length}/8</div>
          <GoldButton variant="ghost" size="sm" onClick={onLeave}>Abbandona ↩</GoldButton>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        <div style={{ flex: 1, position: 'relative', padding: 20 }}>
          <div style={{ position: 'relative', width: '100%', height: '100%', maxWidth: 980, margin: '0 auto' }}>
            <div style={{ position: 'absolute', inset: '10% 4%', borderRadius: '50%', background: 'green', border: '8px solid #3a2818' }}>
              ...
              <div style={{ position: 'absolute', top: '42%', left: '50%', transform: 'translate(-50%, -50%)', display: 'flex', gap: 6 }}>
                {[0, 1, 2, 3, 4].map(i => (
                  <div key={i}>{community[i] ? <Card card={community[i]} size="md" cardBack={cardBack} /> : <div style={{ width: 52, height: 74, border: '1px dashed rgba(212,175,55,0.1)' }} />}</div>
                ))}
              </div>
            </div>

            {seats.map((s, idx) => (
              <Seat key={idx} seat={s} pos={{ left: `${10 + idx * 11}%`, top: idx < 4 ? '15%' : '85%' }} isHero={s && s.is_me} dealerBtn={s && s.is_dealer} cardBack={cardBack} />
            ))}

            <div style={{ position: 'absolute', bottom: 20, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 8, alignItems: 'center' }}>
              <GoldButton variant="ghost" size="sm" onClick={() => handleAction('fold')}>Fold</GoldButton>
              <GoldButton variant="ghost" size="sm" onClick={() => handleAction('call', { amount: tableState?.to_call ?? 0 })}>Call €{tableState?.to_call ?? 0}</GoldButton>
              <GoldButton size="sm" onClick={() => handleAction('raise', { amount: raiseAmt })}>Raise €{raiseAmt}</GoldButton>
              <input type="range" min={tableState?.min_raise ?? 40} max={mySeat?.stack ?? 1000} value={raiseAmt} onChange={e => setRaiseAmt(+e.target.value)} style={{ width: 140 }} />
              <button onClick={() => setRaiseAmt(Math.min(mySeat?.stack ?? 1000, 1250))}>ALL</button>
            </div>
          </div>
        ...
        <div style={{ width: 300, borderLeft: '1px solid rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ padding: 10, borderTop: '1px solid rgba(0,0,0,0.04)' }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <input value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Scrivi..." style={{ flex: 1, padding: 8 }} />
              <GoldButton size="sm" onClick={handleSendChat}>Invia</GoldButton>
            </div>
          </div>
        </div>
      ...
      {showBuyin && <BuyinDialog onConfirm={(seat, amount) => { if (joinSeat) joinSeat(seat, amount); setShowBuyin(false); }} onClose={() => setShowBuyin(false)} />}
    </div>
  );
}
import { GoldButton } from './Shell';
...
export default function PokerTable({ cardBack = 'ridotto', onLeave }) {
  const { tableState, mySeat, myCards, sendAction, joinSeat, leaveSeat } = usePokerTable();
  const { messages, sendMessage } = useTableChat();
  const [pot, setPot] = useState(tableState?.pot ?? 0);
  const [community, setCommunity] = useState([]);
  const [phase, setPhase] = useState('preflop');
  const [raiseAmt, setRaiseAmt] = useState(40);
  const [log, setLog] = useState([

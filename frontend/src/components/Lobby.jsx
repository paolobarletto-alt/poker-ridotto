      ...
      <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 32 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{
            ...
            color: '#F5F1E8', letterSpacing: '-0.02em', lineHeight: 1.05, marginBottom: 10,
          }}>
            Gran Premio<br/>
        ...
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ display: 'flex', gap: 28, marginBottom: 16, justifyContent: 'flex-end' }}>
            {[['GARANTITO', '€50.000'], ['BUY-IN', '€250'], ['INIZIA', '20:30']].map(([label, val]) => (
              <div key={label}>
                <div style={{ fontSize: 9, letterSpacing: '0.2em', color: 'rgba(245,241,232,0.5)', marginBottom: 4 }}>{label}</div>
                <div style={{ fontFamily: 'Playfair Display, serif', fontSize: 26, color: label === 'GARANTITO' ? '#D4AF37' : '#F5F1E8', fontWeight: 500 }}>{val}</div>
              </div>
            ))}
          </div>
...
function FastCards({ data, onJoin }) {
  return (
    <div style={{ margin: '0 32px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
      {data.map(f => (
        <div key={f.id} style={{
...
export default function Lobby({ view = 'lobby' }) {
  ...
  const joinTable = () => navigate('/table/active');
  ...
  if (view === 'tornei') {
    return (
      <div style={{ paddingBottom: 40 }}>
        ...
        />
        <div style={{ height: 20 }} />
        <TorneiTable data={TORNEI} onJoin={joinTable} />
      </div>
    );
  }
  ...
  if (view === 'cash') {
    return (
      <div style={{ paddingBottom: 40 }}>
        ...
        />
        <div style={{ height: 20 }} />
        <CashTable data={CASH_TABLES} onJoin={joinTable} />
      </div>
    );
  }
  ...
  if (view === 'sitgo') {
    return (
      <div style={{ paddingBottom: 40 }}>
        ...
        />
        <div style={{ height: 20 }} />
        <SitGoGrid data={SITNGO} onJoin={joinTable} />
      </div>
    );
  }
  ...
  if (view === 'fast') {
    return (
      <div style={{ paddingBottom: 40 }}>
        <TopBar subtitle="FAST · POOL DI GIOCATORI" title="Fast Poker" actions={null} />
        ...
        <FastCards data={FAST} onJoin={joinTable} />
      </div>
    );
  }
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      />
      <TorneiTable data={TORNEI.slice(0, 3)} onJoin={joinTable} />
      ...
      />
      <CashTable data={CASH_TABLES.slice(0, 4)} onJoin={joinTable} />
      ...
      />
      <SitGoGrid data={SITNGO.slice(0, 3)} onJoin={joinTable} />
    </div>
  );
}
